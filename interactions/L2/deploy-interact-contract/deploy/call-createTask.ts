const startTimeH: number = new Date().getTime();
...
export default async function (hre: HardhatRuntimeEnvironment) {
  ...
  for (let i: number = 0; i < 100; i++) {
    var tx = await contract.createTask(taskHash,amount);
  }
  ...
  const endTimeH: number = new Date().getTime();
  const timeDiffH: number = endTimeH - startTimeH;
  console.log(timeDiffH);
