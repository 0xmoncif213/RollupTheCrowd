...
contract TaskContract {
    ...
    function  createTask(string calldata _taskHash , uint256 _amount, address _requester ) public {
        // vrify existence of user
        require(accessManagementInstance.isUser(_requester)==true , "only registred users can create task");
        ...
        tasks[_taskHash].requester =_requester; 
    ...
    function createBid(string calldata _taskHash ,string calldata _bidHash , address _worker) public {
        require(tasks[_taskHash].exist==true, "Task doesn't exist");  //task exist 
        require(accessManagementInstance.isUser(_worker)==true , "only registred users can submit bids");
        require(tasks[_taskHash].requester != _worker , "requester can't submit bids for his own task");
        //push worker to task 
        tasks[_taskHash].workers.push(_worker); 
        tasks[_taskHash].bidSubmitter[_bidHash] = _worker ; 
        tasks[_taskHash].workerExist[_worker] = true ;
        //push bid to task 
    ...
    function createAnswer(string calldata _taskHash , string calldata _bidHash, bool _answer , address _requester) public {
        require(tasks[_taskHash].exist==true, "Task doesn't exist"); 
        require(tasks[_taskHash].requester==_requester,"Only requester can answer bids");
    ...
    function submitSolution(string calldata _taskHash , string calldata _bidHash , string calldata _solutionHash , address _worker) public {
        require(tasks[_taskHash].bidSubmitter[_bidHash] == _worker, "worker submit solution to his own bid");
