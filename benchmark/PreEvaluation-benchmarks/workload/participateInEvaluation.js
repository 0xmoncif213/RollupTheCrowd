...
class StoreWorkload extends WorkloadModuleBase {
    ...
    _generateWorkload() {
        ...
        for(let i= 0; i < this.roundArguments.txnPerBatch; i++) {
            this.txIndex++;
            let hash = data[0][this.txIndex+roundIndexes[this.roundArguments.num-1]].hash ; 
            let evaluator = web3.eth.accounts.create();
            let args = {
                contract :this.roundArguments.contract,
                verb: 'participateInEvaluation',
                args: [hash,evaluator.address],
                readOnly: false,
            }
