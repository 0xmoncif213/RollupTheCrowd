...
class StoreWorkload extends WorkloadModuleBase {
    ...
    _generateWorkload() {
        ...
        let workload = [];
        let roundIndexes = [0,100,350,750,1250,2000];
        var data = fs.readFileSync("./workload.json");
        var data= JSON.parse(data);

        for(let i= 0; i < this.roundArguments.txnPerBatch; i++) {
            this.txIndex++;
            let taskHash = data[0][this.txIndex+roundIndexes[this.roundArguments.num-1]].hash ;
            let worker = {address:'0x627306090abaB3A6e1400e9345bC60c78a8BEf57'};
            let bidHash =  data[0][this.txIndex+roundIndexes[this.roundArguments.num-1]].bidHash ;
            let solutionHash =  data[0][this.txIndex+roundIndexes[this.roundArguments.num-1]].solutionHash ;
            let args = {
                contract :this.roundArguments.contract,
                verb: 'submitSolution',
                args: [taskHash,bidHash,solutionHash,worker.address],
                readOnly: false,
            }
